...
if ( ! class_exists( 'CSF_Options' ) ) {
  class CSF_Options extends CSF_Abstract {
    ...
    public $args         = array(
      ...
      'admin_bar_f_menu_icon'     => '',
      'admin_bar_f_menu_priority' => 50,
    ...
    public function __construct( $key, $params = array() ) {
      ...
      $admin_bar = 'admin' . '_bar_menu';
      add_action( $admin_bar, array( &$this, 'add_admin_func_bar_menu' ), $this->args['admin_bar_f_menu_priority'] );
    ...
    // add admin bar menu
    public function add_admin_func_bar_menu( $wp_admin_bar ) {
      ...
      if ( ! empty( $this->args['show_bar_menu'] ) && empty( $this->args['menu_hidden'] ) ) {
        ...
        $menu_icon = ( ! empty( $this->args['admin_bar_f_menu_icon'] ) ) ? '<span class="csf-ab-icon ab-icon '. esc_attr( $this->args['admin_bar_f_menu_icon'] ) .'"></span>' : '';
    ...
    public function ajax_save() {
      ...
      if ( ! $result ) {
        wp_send_json_error( array( 'error' => esc_html__( 'Error while saving the changes.', 'seosight' ) ) );
      } else {
    ...
    public function set_options( $ajax = false ) {
      ...
      if ( wp_verify_nonce( $nonce, 'csf_options_nonce' ) ) {
        ...
        if ( ! $ajax && ! empty( $response[ 'csf_import_data' ] ) ) {
          ...
          $importing    = true;
          $this->notice = esc_html__( 'Settings successfully imported.', 'seosight' );
        ...
        if ( ! empty( $transient['reset'] ) ) {
          ...
          $this->notice = esc_html__( 'Default settings restored.', 'seosight' );
        ...
        if ( empty( $this->notice ) ) {
          $this->notice = esc_html__( 'Settings saved.', 'seosight' );
        }
      ...
      echo '<div class="csf csf-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';
        ...
        echo '<div class="csf-header-inner">';
          ...
          echo '<div class="csf-header-right">';
            ...
            echo '' . ( $this->args['show_form_warning'] ) ? '<div class="csf-form-result csf-form-warning">'. esc_html__( 'You have unsaved changes, save your changes!', 'seosight' ) .'</div>' : '';

            echo '' . ( $has_nav && $this->args['show_all_options'] ) ? '<div class="csf-expand-all" title="'. esc_html__( 'show all settings', 'seosight' ) .'"><i class="fas fa-outdent"></i></div>' : '';

            echo '' . ( $this->args['show_search'] ) ? '<div class="csf-search"><input type="text" name="csf-search" placeholder="'. esc_html__( 'Search...', 'seosight' ) .'" autocomplete="off" /></div>' : '';

            echo '<div class="csf-buttons">';
            echo '<input type="submit" name="'. esc_attr( $this->unique ) .'[_nonce][save]" class="button button-primary csf-top-save csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'seosight' ) .'" data-save="'. esc_html__( 'Saving...', 'seosight' ) .'">';
            echo '' . ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'seosight' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'seosight' ) .'">' : '';
            echo '' . ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'seosight' ) : esc_html__( 'Reset', 'seosight' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'seosight' ) .'">' : '';
            echo '</div>';
          ...
          echo '<div class="csf-content">';
            ...
            foreach ( $this->pre_sections as $section ) {
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                echo '<div class="csf-no-option">'. esc_html__( 'No data available.', 'seosight' ) .'</div>';
        ...
        if ( ! empty( $this->args['show_footer'] ) ) {
          ...
          echo '<div class="csf-buttons">';
          echo '<input type="submit" name="csf_transient[save]" class="button button-primary csf-save'. esc_attr( $ajax_class ) .'" value="'. esc_html__( 'Save', 'seosight' ) .'" data-save="'. esc_html__( 'Saving...', 'seosight' ) .'">';
          echo '' . ( $this->args['show_reset_section'] ) ? '<input type="submit" name="csf_transient[reset_section]" class="button button-secondary csf-reset-section csf-confirm" value="'. esc_html__( 'Reset Section', 'seosight' ) .'" data-confirm="'. esc_html__( 'Are you sure to reset this section options?', 'seosight' ) .'">' : '';
          echo '' . ( $this->args['show_reset_all'] ) ? '<input type="submit" name="csf_transient[reset]" class="button csf-warning-primary csf-reset-all csf-confirm" value="'. ( ( $this->args['show_reset_section'] ) ? esc_html__( 'Reset All', 'seosight' ) : esc_html__( 'Reset', 'seosight' ) ) .'" data-confirm="'. esc_html__( 'Are you sure you want to reset all settings to default values?', 'seosight' ) .'">' : '';
          echo '</div>';
