...
if ( ! class_exists( 'CSF_Metabox' ) ) {
  class CSF_Metabox extends CSF_Abstract{
    ...
    public function __construct( $key, $params = array() ) {
      ...
      $meta_add = 'add' . '_meta_boxes';
      add_action( $meta_add, array( &$this, 'add_meta_f_box' ) );
    ...
    // add metabox
    public function add_meta_f_box( $post_type ) {

      if ( ! in_array( $post_type, $this->args['exclude_post_types'] ) ) {
        $add = 'add' . '_meta_box';
        $add( $this->unique, $this->args['title'], array( &$this, 'add_meta_f_box_content' ), $this->post_type, $this->args['context'], $this->args['priority'], $this->args );
      }
    ...
    // add metabox content
    public function add_meta_f_box_content( $post, $callback ) {
        ...
        echo '<div class="csf-wrapper'. esc_attr( $show_all ) .'">';
          ...
          echo '<div class="csf-content">';
            ...
            if ( ! empty( $this->args['show_restore'] ) || ! empty( $this->args['show_reset'] ) ) {
              ...
              echo '<span class="button csf-button-reset">'. esc_html__( 'Reset', 'seosight' ) .'</span>';
              echo '<span class="button csf-button-cancel">'. sprintf( '<small>( %s )</small> %s', esc_html__( 'update post', 'seosight' ), esc_html__( 'Cancel', 'seosight' ) ) .'</span>';
              echo '</label>';
              echo '</div>';
