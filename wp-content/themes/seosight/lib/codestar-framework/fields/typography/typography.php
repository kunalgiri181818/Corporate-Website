      ...
      echo '<div class="csf--typography'. esc_attr( $chosen_class ) .'" data-depend-id="'. esc_attr( $this->field['id'] ) .'" data-unit="'. esc_attr( $args['unit'] ) .'" data-line-height-unit="'. esc_attr( $line_height_unit ) .'" data-exclude="'. esc_attr( $args['exclude'] ) .'">';
        ...
        echo '<div class="csf--blocks csf--blocks-selects">';
          ...
          if ( ! empty( $args['font_family'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Font Family', 'seosight' ) .'</div>';
            echo '' . $this->create_select( array( $this->value['font-family'] => $this->value['font-family'] ), 'font-family', esc_html__( 'Select a font', 'seosight' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['backup_font_family'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Backup Font Family', 'seosight' ) .'</div>';
            echo '' . $this->create_select( apply_filters( 'csf_field_typography_backup_font_family', array(
              ...
              'Georgia, serif',
              'Palatino Linotype'
            ) ), 'backup-font-family', esc_html__( 'Default', 'seosight' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_weight'] ) || ! empty( $args['font_style'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Font Style', 'seosight') .'</div>';
            echo '<select class="csf--font-style-select" data-placeholder="Default">';
            echo '<option value="">'. ( ! $this->chosen ? esc_html__( 'Default', 'seosight' ) : '' ) .'</option>';
            ...
            if ( ! empty( $args['extra_styles'] ) ) {
              ...
              echo ( ! $this->chosen ) ? '<div class="csf--title">'. esc_html__( 'Load Extra Styles', 'seosight' ) .'</div>' : '';
              $placeholder = ( $this->chosen ) ? esc_html__( 'Load Extra Styles', 'seosight' ) : esc_html__( 'Default', 'seosight' );
          ...
          if ( ! empty( $args['subset'] ) ) {
            ...
            echo '<div class="csf--title">'. esc_html__( 'Subset', 'seosight' ) .'</div>';
            $subset = ( is_array( $this->value['subset'] ) ) ? $this->value['subset'] : array_filter( (array) $this->value['subset'] );
            echo '' . $this->create_select( $subset, 'subset', esc_html__( 'Default', 'seosight' ), $args['multi_subset'] );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_align'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Align', 'seosight' ) .'</div>';
            echo '' . $this->create_select( array(
              'inherit' => esc_html__( 'Inherit', 'seosight' ),
              'left'    => esc_html__( 'Left', 'seosight' ),
              'center'  => esc_html__( 'Center', 'seosight' ),
              'right'   => esc_html__( 'Right', 'seosight' ),
              'justify' => esc_html__( 'Justify', 'seosight' ),
              'initial' => esc_html__( 'Initial', 'seosight' )
            ), 'text-align', esc_html__( 'Default', 'seosight' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['font_variant'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Font Variant', 'seosight' ) .'</div>';
            echo '' . $this->create_select( array(
              'normal'         => esc_html__( 'Normal', 'seosight' ),
              'small-caps'     => esc_html__( 'Small Caps', 'seosight' ),
              'all-small-caps' => esc_html__( 'All Small Caps', 'seosight' )
            ), 'font-variant', esc_html__( 'Default', 'seosight' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_transform'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Transform', 'seosight' ) .'</div>';
            echo '' . $this->create_select( array(
              'none'       => esc_html__( 'None', 'seosight' ),
              'capitalize' => esc_html__( 'Capitalize', 'seosight' ),
              'uppercase'  => esc_html__( 'Uppercase', 'seosight' ),
              'lowercase'  => esc_html__( 'Lowercase', 'seosight' )
            ), 'text-transform', esc_html__( 'Default', 'seosight' ) );
            echo '</div>';
          }
          ...
          if ( ! empty( $args['text_decoration'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Text Decoration', 'seosight' ) .'</div>';
            echo '' . $this->create_select( array(
              'none'               => esc_html__( 'None', 'seosight' ),
              'underline'          => esc_html__( 'Solid', 'seosight' ),
              'underline double'   => esc_html__( 'Double', 'seosight' ),
              'underline dotted'   => esc_html__( 'Dotted', 'seosight' ),
              'underline dashed'   => esc_html__( 'Dashed', 'seosight' ),
              'underline wavy'     => esc_html__( 'Wavy', 'seosight' ),
              'underline overline' => esc_html__( 'Overline', 'seosight' ),
              'line-through'       => esc_html__( 'Line-through', 'seosight' )
            ), 'text-decoration', esc_html__( 'Default', 'seosight' ) );
            echo '</div>';
          }
        ...
        echo '<div class="csf--blocks csf--blocks-inputs">';
          ...
          if ( ! empty( $args['font_size'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Font Size', 'seosight' ) .'</div>';
          ...
          if ( ! empty( $args['line_height'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Line Height', 'seosight' ) .'</div>';
            echo '<div class="csf--input-wrap">';
          ...
          if ( ! empty( $args['letter_spacing'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Letter Spacing', 'seosight' ) .'</div>';
            echo '<div class="csf--input-wrap">';
          ...
          if ( ! empty( $args['word_spacing'] ) ) {
            echo '<div class="csf--block">';
            echo '<div class="csf--title">'. esc_html__( 'Word Spacing', 'seosight' ) .'</div>';
            echo '<div class="csf--input-wrap">';
        ...
        if ( ! empty( $args['color'] ) ) {
          ...
          echo '<div class="csf--title">'. esc_html__( 'Font Color', 'seosight' ) .'</div>';
        ...
        if ( ! empty( $args['custom_style'] ) ) {
          ...
          echo '<div class="csf--title">'. esc_html__( 'Custom Style', 'seosight' ) .'</div>';
      ...
      if ( ! wp_script_is( 'csf-webfontloader' ) ) {
        ...
        if ( ! empty( $customwebfonts ) ) {
          $webfonts['custom'] = array(
            'label' => esc_html__( 'Custom Web Fonts', 'seosight' ),
            'fonts' => $customwebfonts
          );
        }

        $webfonts['safe'] = array(
          'label' => esc_html__( 'Safe Web Fonts', 'seosight' ),
        ...
        $webfonts['google'] = array(
          'label' => esc_html__( 'Google Web Fonts', 'seosight' ),
